...
// Desktop fragment shader - 4 FBM iterations (down from 6)
const fragmentShaderDesktop = `
  ...
  void main() {
    ...
    float warp = u_warp;
    vec2 q = vec2(fbm(vec3(p, 1.0), time, warp), fbm(vec3(p + 5.2, 1.0), time, warp));
    vec2 r = vec2(fbm(vec3(p + 4.0 * q + vec2(1.7, 9.2), 1.0), time, warp), fbm(vec3(p + 4.0 * q + vec2(8.3, 2.8), 1.0), time, warp));
    float f = fbm(vec3(p + 3.5 * r, 1.0), time, warp);
    vec3 l_col1 = vec3(0.957, 0.945, 0.925);
    vec3 l_col2 = vec3(0.831, 0.863, 0.910);
    vec3 l_col3 = vec3(0.478, 0.549, 0.627);
    vec3 l_col4 = vec3(0.094, 0.094, 0.094);
    vec3 d_col1 = vec3(0.220, 0.230, 0.250);
    vec3 d_col2 = vec3(0.165, 0.175, 0.200);
    vec3 d_col3 = vec3(0.100, 0.110, 0.140);
    vec3 d_col4 = vec3(0.040, 0.042, 0.055);
...
// Mobile shader - simple value noise, very light
const fragmentShaderMobile = `
  ...
  void main() {
    ...
    vec2 p = aspect * (gl_FragCoord.xy / u_res - 0.5) * u_scale;
    float time = u_time * u_speed;
    float n = smoothNoise(p * 2.0 + time * 0.3);
    n += 0.5 * smoothNoise(p * 4.0 - time * 0.2 + 1.7);
    n += 0.25 * smoothNoise(p * 8.0 + time * 0.1 + 5.2);
    n /= 1.75;
    vec3 l_col1 = vec3(0.957, 0.945, 0.925);
    vec3 l_col4 = vec3(0.094, 0.094, 0.094);
    vec3 d_col1 = vec3(0.220, 0.230, 0.250);
    ...
    vec3 col1 = mix(l_col1, d_col1, u_dark_mode);
    vec3 col4 = mix(l_col4, d_col4, u_dark_mode);
    vec3 color = mix(col1, col4, smoothstep(0.3, 0.7, n));
...
export default function ShaderCanvas() {
  ...
  useEffect(() => {
    // Dynamically import Three.js - reduces initial bundle size
    const initThree = async () => {
      try {
        ...
        const uniforms: Record<string, THREE.IUniform> = {
          ...
          u_scale: { value: isMobile ? 3.5 : 2.5 },
          u_speed: { value: isMobile ? 0.03 : 0.05 },
          u_warp: { value: 0.45 },
  ...
  // Update dark mode uniform
  useEffect(() => {
    // No direct reference to uniforms after refactor; theme reactivity handled less ideally
    // but theme will be correct on re-mount
  }, [theme]);
